        ...
        <div className='grid items-center gap-3 px-5'>
            {plancontent?.map((val, i) => (
                <div key={i} className="flex items-center justify-start gap-5">
                    ...
                    <div className='grid items-center'>
                        <h1 className='font-normal text-lg text-gray-900 filter drop-shadow-lg lg:text-base hover:text-purple-500'>{val.text}</h1>
                    </div>
                </div>
            ))}
        </div>
        <div className='grid items-center justify-items-center mt-7'>
            <button type='button' className='button-purple text-base'>{buttonText}</button>
        </div>
      </div>
   </>
  )
